...
public sealed class JobsController(FirestoreDataService database) : ControllerBase
{
    ...
    [HttpDelete("{id}")]
    public async Task<IActionResult> DeleteJob(string id, [FromQuery] string companyId, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(companyId)) return ValidationProblem("CompanyId is required.");
        if (!await database.DeleteJobAsync(id, companyId, cancellationToken))
        {
    ...
    {
        if (string.IsNullOrWhiteSpace(dto.CompanyId) || string.IsNullOrWhiteSpace(dto.Company) || string.IsNullOrWhiteSpace(dto.Role) ||
            string.IsNullOrWhiteSpace(dto.WorkAddress) || dto.Latitude is < -90 or > 90 ||
            dto.Longitude is < -180 or > 180)
        {
            return ValidationProblem("CompanyId, company, role, a valid address and valid coordinates are required.");
        }
